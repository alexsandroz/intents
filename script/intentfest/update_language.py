...
def run() -> int:
    ...
    base_sentence_dir = SENTENCE_DIR / "en"
    update_sentence_dir = SENTENCE_DIR / args.language

    for base_sentence_file in os.listdir(base_sentence_dir):
        base_sentence_file_path = os.path.join(base_sentence_dir, base_sentence_file)
        update_sentence_file_path = os.path.join(update_sentence_dir, base_sentence_file)

        with open(base_sentence_file_path, 'r') as f_base:
            dados_base = yaml.load(f_base)

        # Checks if the update file already exists
        if os.path.exists(update_sentence_file_path):
            with open(update_sentence_file_path, 'r') as f_update:
        ...
        yaml.width = 1024
        with open(update_sentence_file_path, 'w') as f_saida:
